...
/**
 * SETUP WIZARD - Lógica para avançar/voltar na configuração inicial
 */
...
function showSetupStep(step) {
  ...
  // Popula o resumo no Passo 3
  if (step === 3) {
    const key = document.getElementById('setupApiKey')?.value || '';
    const cid = document.getElementById('setupClientId')?.value || '';
    const sumKey = document.getElementById('sumApiKey');
    const sumCid = document.getElementById('sumClientId');
    if (sumKey) sumKey.textContent = key.length > 10 ? key.substring(0, 10) + '...' : key;
    if (sumCid) sumCid.textContent = cid || 'Não configurado (Modo Demo)';
  }
}
...
function setupNext() {
  if (_currentSetupStep === 1) {
    ...
    if (!key) { showNotif('error', '⚠️', 'A chave da API é obrigatória para continuar.'); return; }
    showSetupStep(2);
  } else if (_currentSetupStep === 2) {
    showSetupStep(3);
  } else {
    finishSetup();
  }
...
function setupBack() {
  if (_currentSetupStep > 1) showSetupStep(_currentSetupStep - 1);
}
...
function finishSetup() {
  const cfg = {
    ...
    autoClassify: true,
    useOutlookFolders: true,
    batchSize: 20
  };
  localStorage.setItem('mailmind_config', JSON.stringify(cfg));
  document.getElementById('setupScreen').classList.add('hidden');
  if (typeof loadApp === 'function') loadApp(cfg);
  showNotif('success', '🚀', 'Configuração concluída com sucesso!');
}

function initResize() { /* Mantido para compatibilidade, implementar se necessário */ }