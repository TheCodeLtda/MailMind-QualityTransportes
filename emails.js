...
async function renderEmailBody(email) {
  ...
  if (html) {
    area.innerHTML = '<div class="body-loading"><div class="spinner"></div> Processando...</div>';
    ...
    iframe.onload = () => {
      ...
      if (doc && doc.body) iframe.style.height = (doc.body.scrollHeight + 30) + 'px';
    };
  } else {
...
async function processEmailHtml(email) {
  ...
  if (state.accessToken && /cid:/i.test(html)) {
    try {
      ...
      atts.forEach(a => {
        if (a.contentBytes && a.contentId) cidMap.set(a.contentId.replace(/^<|>$/g, ''), `data:${a.contentType};base64,${a.contentBytes}`);
      });
      doc.querySelectorAll('[src^="cid:"]').forEach(img => {
        const cid = img.getAttribute('src').substring(4);
        if (cidMap.has(cid)) img.setAttribute('src', cidMap.get(cid));
      });
    } catch {}
  }
...
async function loadAndRenderAttachments(email) {
  ...
  if (!area) return;
  const atts = await fetchAttachments(email.id);
  const real = atts.filter(a => !a.isInline);
  if (!real.length) { area.innerHTML = ''; return; }
  area.innerHTML = `
    <div class="attachments-bar">
      <div class="attachments-label">Anexos (${real.length})</div>
      <div class="attachments-list">${real.map(a => `
        <div class="attachment-item">
          <span class="attach-file-icon">${getAttachIcon(a.name)}</span>
          <div class="attach-info">
            <div class="attach-name">${escHtml(a.name)}</div>
            <div class="attach-size">${Math.round(a.size/1024)} KB</div>
          </div>
          <a class="attach-download" href="data:${a.contentType};base64,${a.contentBytes}" download="${escHtml(a.name)}">⬇</a>
        </div>`).join('')}
      </div>
    </div><div class="detail-divider"></div>`;
}
...
function applyFilters() {
  ...
  if (search) emails = emails.filter(e => e.subject.toLowerCase().includes(search) || e.from.toLowerCase().includes(search));
...
function renderPagination() {
  ...
  bar.innerHTML = `
    <div class="pagination-info">Página ${state.page.current}</div>
...
function goNextPage() { if (state.page.nextLink) { state.page.current++; fetchEmails(state.page.nextLink); } }
function goPrevPage() { if (state.page.current > 1) { state.page.current--; fetchEmails(); } }
